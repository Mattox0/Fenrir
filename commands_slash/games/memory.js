...
module.exports = {
    ...
    async execute(...params) {
        ...
        const ask = new MessageEmbed()
            .setColor('#2f3136')
            .setDescription(`Tu as ${time} secondes pour restituer la suite de mots !\n\n\`${suite.join('` `')}\``)
            .setFooter({text:`LMT-Bot ・ Aujourd'hui à ${date.toLocaleTimeString().slice(0,-3)}`, iconURL:'https://cdn.discordapp.com/avatars/784943061616427018/2dd6a7254954046ce7aa31c42f1147e4.webp'})
        await interaction.reply({embeds:[ask]});
        await wait(1000*time);
        const reponse = new MessageEmbed()
            .setColor('#2f3136')
            .setDescription('<a:LMT__arrow:831817537388937277> **Tu as bien retenu ? Je t\'écoute !**')
        ...
        interaction.editReply({embeds:[reponse]}).then(msg => {
            ...
            collector.on('end', collected => {
                if (!collected.first()) {
                    ...
                    .setDescription(`<a:LMT__arrow:831817537388937277> **Les 30 secondes sont écoulés**`)
                    .setFooter({text:`LMT-Bot ・ Aujourd'hui à ${date.toLocaleTimeString().slice(0,-3)}`, iconURL:'https://cdn.discordapp.com/avatars/784943061616427018/2dd6a7254954046ce7aa31c42f1147e4.webp'})
                    return msg.edit({ embeds: [delai], components: [] })
                }
                if (collected.first().content === suite.join(' ')) {
                    const win = new MessageEmbed()
                        .setDescription('<a:LMT__arrow:831817537388937277> **Bien joué, tu as réussi a retrouver ma phrase !**')
                        .setFooter({text:`LMT-Bot ・ Aujourd'hui à ${date.toLocaleTimeString().slice(0,-3)}`, iconURL:'https://cdn.discordapp.com/avatars/784943061616427018/2dd6a7254954046ce7aa31c42f1147e4.webp'})
                    ...
                    const fail = new MessageEmbed()
                        .setDescription(`<a:LMT__arrow:831817537388937277> **Eh non, tu n\'as pas retrouvé la phrase !**\n\n> ${phrase}\n\n> ${phraseV}`)
                        .setFooter({text:`LMT-Bot ・ Aujourd'hui à ${date.toLocaleTimeString().slice(0,-3)}`, iconURL:'https://cdn.discordapp.com/avatars/784943061616427018/2dd6a7254954046ce7aa31c42f1147e4.webp'})
