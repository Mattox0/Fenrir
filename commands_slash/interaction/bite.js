...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option => option.setName('utilisateur').setDescription('La personne que tu veux mordre | Si rien -> personne aléatoire').setRequired(false)),
        ...
        try {
            ...
            .then(data => {
                const bite = new MessageEmbed()
                    .setColor('#2f3136')
                    .setDescription(`<a:LMT__arrow:831817537388937277> ${interaction.member.nickname ? interaction.member.nickname : interaction.member.user.username} **mord** ${person.nickname ? person.nickname : person.user.username}`)
                    .setImage(data["url"])
