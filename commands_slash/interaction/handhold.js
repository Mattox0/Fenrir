...
module.exports = {
    ...
    async execute(...params) {
        ...
        try {
            ...
            .then(r => r.json())
            .then(data => {
                const handhold = new MessageEmbed()
                    .setColor('#2f3136')
                    .setDescription(`<a:LMT__arrow:831817537388937277> ${interaction.member.nickname ? interaction.member.nickname : interaction.member.user.username} **serre** la main de ${person.nickname ? person.nickname : person.user.username}`)
                    .setImage(data["url"])
