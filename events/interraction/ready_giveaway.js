...
module.exports = {
    ...
    execute(...params) {
        let db = params[0];
        let client = params[1];
        db.each("SELECT * FROM giveaways", (err, res) => {
            if (res.past === 1) {
                ...
                new schedule.scheduleJob(fin, async function() {
                    db.get('SELECT * FROM giveaways WHERE message_id = ?',res.message_id, async (err,result) => {
                        if (!result) {
                            return 
                        }
                        if (result.past === 1) {
                        ...
                        db.run('UPDATE giveaways SET past = ? WHERE message_id = ?',true,result.message_id, err => {
                            if (err) console.log(err)
                        })
                        let fin = new Date(result.duration * 1000)
                        fin.setDate(fin.getDate() + 1);
