            ...
            if (res.threadCreate) {
                ...
                if (!deletionLog) {
                    const event = new EmbedBuilder()
                        .setColor('#2f3136')
                        .setDescription(`<a:LMT_arrow:1065548690862899240> **Un fil a été crée !**\n\n**Nom** : \`${thread.name}\`\n\n**ID** : ${thread.id}\n\n**Date de création :** <t:${Math.ceil(date / 1000)}:F>`)
                    ...
                    const event = new EmbedBuilder()
                        ...
                        .setDescription(`<a:LMT_arrow:1065548690862899240> **Un fil a été crée par ${deletionLog.executor} !**\n\n**Nom** : \`${thread.name}\`\n\n**ID** : ${thread.id}\n\n**Date de création :** <t:${Math.ceil(date / 1000)}:F>`)
                        .setFooter({text:`LMT-Bot ・ Aujourd'hui à ${date.toLocaleTimeString().slice(0,-3)}`, iconURL:'https://cdn.discordapp.com/avatars/784943061616427018/2dd6a7254954046ce7aa31c42f1147e4.webp'})
                    return chann.send({embeds:[event]});
                }
            }
