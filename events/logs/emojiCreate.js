...
module.exports = {
	...
	async execute(...params) {
        ...
        let db = params[1];
        let date = new Date()
        db.get("SELECT emojiCreate, logs_id FROM logs WHERE guild_id = ?",emoji.guild.id, async (err, res) => {
            ...
            if (res.emojiCreate) {
                ...
                if (!deletionLog) {
                    const event = new MessageEmbed()
                        .setColor('#2f3136')
                        .setDescription(`<a:LMT__arrow:831817537388937277> **L'emoji ${emoji} a été crée !**\n\n**Nom** : \`${emoji.name}\`\n\n**ID** : ${emoji.id}\n\n**Date de création :** <t:${Math.ceil(date / 1000)}:F>`)
                    ...
                    const event = new MessageEmbed()
                        ...
                        .setDescription(`<a:LMT__arrow:831817537388937277> **L'emoji ${emoji} a été crée par ${deletionLog.executor} !**\n\n**Nom** : \`${emoji.name}\`\n\n**ID** : ${emoji.id}\n\n**Date de création :** <t:${Math.ceil(date / 1000)}:F>`)
                        .setFooter({text:`LMT-Bot ・ Aujourd'hui à ${date.toLocaleTimeString().slice(0,-3)}`, iconURL:'https://cdn.discordapp.com/avatars/784943061616427018/2dd6a7254954046ce7aa31c42f1147e4.webp'})
                    return chann.send({embeds:[event]});
                }
            }
