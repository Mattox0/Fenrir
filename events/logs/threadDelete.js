...
module.exports = {
	...
	async execute(...params) {
        ...
        let db = params[1];
        let date = new Date()
        db.get("SELECT threadDelete, logs_id FROM logs WHERE guild_id = ?",thread.guild.id, async (err, res) => {
            ...
            if (res.threadDelete) {
                ...
                if (!deletionLog) {
                    const event = new MessageEmbed()
                        .setColor('#2f3136')
                        .setDescription(`<a:LMT__arrow:831817537388937277> **Un fil a été supprimé !**\n\n**Nom** : \`${thread.name}\`\n\n**ID** : ${thread.id}\n\n**Date de suppression :** <t:${Math.ceil(date / 1000)}:F>`)
                    ...
                    const event = new MessageEmbed()
                        ...
                        .setDescription(`<a:LMT__arrow:831817537388937277> **Un fil a été supprimé par ${deletionLog.executor} !**\n\n**Nom** : \`${thread.name}\`\n\n**ID** : ${thread.id}\n\n**Date de suppression :** <t:${Math.ceil(date / 1000)}:F>`)
                        .setFooter({text:`LMT-Bot ・ Aujourd'hui à ${date.toLocaleTimeString().slice(0,-3)}`, iconURL:'https://cdn.discordapp.com/avatars/784943061616427018/2dd6a7254954046ce7aa31c42f1147e4.webp'})
                    return chann.send({embeds:[event]});
                }
            }
