...
module.exports = {
  ...
  async execute(interaction: any) {
    ...
    fetch(`https://api.bunnies.io/v2/loop/random/?media=gif,png`)
      ...
      .then((data: any) => {
        const bunny: EmbedBuilder = new EmbedBuilder()
          .setColor('#2f3136')
          .setTimestamp()
          .setFooter({text:`${process.env.BOT_NAME}`, iconURL:process.env.ICON_URL})
        if (data.media.gif) {
          bunny.setImage(data.media.gif);
        } else {
          bunny.setImage(data.media.poster);
        }
        return interaction.editReply({embeds:[bunny]});
      })
      .catch(async (e: any) => {
        console.log('error', e);
