...
module.exports = {
    ...
    async execute(...params) {
        ...
        try {
            ...
            .then(r => r.json())
            .then(data => {
                const cry = new EmbedBuilder()
                    .setColor('#2f3136')
                    .setDescription(`<a:LMT_arrow:1065548690862899240> ${interaction.member.nickname ? interaction.member.nickname : interaction.member.user.username} **pleure** auprès de ${person.nickname ? person.nickname : person.user.username}`)
                    .setImage(data["url"])
